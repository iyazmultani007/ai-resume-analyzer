...
async function loadPdfJs(): Promise<any> {
    ...
    loadPromise = import("pdfjs-dist/build/pdf.mjs").then(async (lib) => {
        try {
            // Import worker code and create blob URL
            const workerCode = await import("pdfjs-dist/build/pdf.worker.mjs?raw");
            const blob = new Blob([workerCode.default], { type: 'application/javascript' });
            const workerUrl = URL.createObjectURL(blob);
            lib.GlobalWorkerOptions.workerSrc = workerUrl;
        } catch (error) {
            console.warn('Failed to load worker, using CDN fallback', error);
            // Fallback to CDN
